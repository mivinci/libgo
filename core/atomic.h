...
#define Atomic_add(p, v) atomic_add_fetch_explicit(p, v, memory_order_acquire);
#else
#define Atomic_cas(p, old, new)                                                \
  __atomic_compare_exchange_n(p, old, new, 1, __ATOMIC_RELEASE,                \
                              __ATOMIC_RELAXED)
#define Atomic_load(p) __atomic_load_n(p, __ATOMIC_ACQUIRE)
#define Atomic_store(p, v) __atomic_store_n(p, v, __ATOMIC_RELEASE)
#define Atomic_add(p, v) __atomic_add_fetch(p, v, __ATOMIC_ACQUIRE)
#endif
